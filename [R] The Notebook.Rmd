...
2.20.[vector] floor ceiling threshold
...
2.20.1.[vector] length round sample 
...
4.2.20.[vector] histogram skewness check
...
4.2.20.[vector] scatter plot empirical cumulative distribution = probability distribution of an outcome to happen
...
4.2.20.[vector] box plot 

```{r}
boxplot(heights_sex_male$height,heights_sex_female$height)
```

4.2.20.[vector] QQ plot  = normality of probability distribution test
...
2.21.[vector] random distribution of factor 
...
2.21.1.[vector] random distribution of factor sample repeatedly 
...
sample <- sample(female_mice_weight_diet_chow,5)# use the sample() function immediately after setting a seed,  to get the same sample again and again 
abs(mean(sample)-mean(female_mice_weight_population)) #absolute value  of the difference between the average of the sample and the average of all the values
set.seed(2) 
sample<- sample(female_mice_weight_diet_chow,5)
abs(mean(sample)-mean(female_mice_weight_population))
set.seed(10) 
sample<- sample(female_mice_weight_diet_chow,5)
abs(mean(sample)-mean(female_mice_weight_population))
```
