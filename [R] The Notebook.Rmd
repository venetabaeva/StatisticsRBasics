...
controlsXGaze<- filter(dfPonzo, stimulusNr == c(1,3,13,15,25,27)) %>% select(yGaze) %>% unlist
```
...
1.[scatterplot] empirical cumulative distribution - ECD ; shows % of individuals below a threshold

```{r}
samplexGazeS<- seq(floor(min(samplexGaze)),ceiling(max(samplexGaze)),0.1)
plo2t(samplexGazeS,ecdf(samplexGaze)(samplexGazeS),type="l",xlim=c(0.0,2.0),
...
1.[QQ] check whether mean agrees with pnorm,if TRUE, then the normal distribution is very good approximation
...
1.[boxplot]use for non normally distributed data
